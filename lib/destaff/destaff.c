...
//esp_err_t deStaff(uint8_t *input, size_t *length) 
//int deStaff(const uint8_t* input, size_t len);
int deStaff(uint8_t *input, size_t len)             // input одновременно и output
{
    ...
    while (read_idx < len)    //      initial_len) 
    {
        // Проверяем текущий и следующий байт, если есть
        if (input[read_idx] == DLE && (read_idx + 1 < len))     //initial_len)) 
        {
